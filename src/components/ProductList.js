...
function ProductList() {
  ...
  const [activeCat, setActiveCat] = useState(params.category ? params.category : "");
        ...
        <div className="tab-content" data-aos="fade-up" data-aos-delay={300}>
          <div className="tab-pane fade active show" id="#Home-Linen">
            ...
            <div className="my-5 d-flex justify-content-center">
              <input className="form-control width50-100" placeholder="Seach Product" onChange={(e)=>setSearchKey(e.target.value)}/>
              <i className="fa fa-search" style={{ position: "relative", top: "10px", right: "30px" }}></i>
            </div>
            <div className="row gy-5">
              {productListArr
                // ?.filter((v) => v?.category === activeCat)
                .map((v, i) => (
                  <ProductCard key={i} v={v} />
                ))}
            </div>
          </div>
        </div>
